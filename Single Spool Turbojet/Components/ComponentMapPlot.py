...
if type == "C":
    ...
    for i in range(Num_refinement-1):
        ...
        plt.text(refined_map["m"][int(pos_N[i])][1],refined_map["pi"][int(pos_N[i])][1]-0.6,str(np.round(N_refined[int(pos_N[i])],2)),size = 10)

    plt.grid(True,linewidth=0.25,color='k',which="major")
    plt.grid(True,linewidth=0.15,linestyle=':',color='k',which="minor")
    plt.minorticks_on()

    plt.xlim([np.floor(np.min(refined_map["m"])-1),np.ceil(np.max(refined_map["m"])+1)])
